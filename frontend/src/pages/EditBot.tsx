...
const EditBot: React.FC<defaultPageProps> = props => {
	...
	function handleName() {
		if (name && bot) {
			changeName(token, bot.id, name).catch(err => console.log(err))
		}
	}

	function handleBrain() {
		if (brain && bot) {
			changeBotBrain(token, bot.id, brain).catch(err => console.log(err))
		}
	}

	function handleSocket() {
		if (socket && bot) {
			const access : ServiceAccess = {socket: socket, discord: bot.serviceAccess.discord, slack: bot.serviceAccess.slack, mastodon: bot.serviceAccess.mastodon}
	...
	function handleDiscord() {
		if (discord && bot) {
			const access : ServiceAccess = {socket: bot.serviceAccess.socket, discord: discord, slack: bot.serviceAccess.slack, mastodon: bot.serviceAccess.mastodon}
			changeBotServices(token, bot.id, access).catch(err => console.log(err))
		}
	}
	...
	function handleSlack() {
		if (slack && bot) {
			const access : ServiceAccess = {socket: bot.serviceAccess.socket, discord: bot.serviceAccess.discord, slack: slack, mastodon: bot.serviceAccess.mastodon}
			changeBotServices(token, bot.id, access).catch(err => console.log(err))
		}
	}
		...
		<div>
			<div className="main">
				...
				<div className="bot-name">
					...
					<button className="bot-name-button" onClick={() => handleName()}>Update</button>
				</div>
				...
				<div className="bot-brain">
					...
					<button className="bot-brain-button" onClick={() => handleBrain()}>Update</button>
				</div>
				...
				<div className="bot-services">
					...
					<div className="bot-services-socket">
						...
						<button className="bot-services-socket-button" onClick={() => handleSocket()}>Update</button>
					</div>
					<div className="bot-services-discord">
						<div className="bot-services-discord-title">Discord :</div>
						<input type="checkbox" checked={discord} onChange={e => setDiscord(e.target.checked)} className="bot-services-discord-input" />
						<button className="bot-services-discord-button" onClick={() => handleDiscord()}>Update</button>
					</div>
					<div className="bot-services-slack">
						...
						<button className="bot-services-slack-button" onClick={() => handleSlack()}>Update</button>
					</div>
				</div>
			</div>
		</div>
